-- Verify billing/open-bag schema after applying 202606150001_billing_open_bag.sql.
...
column_checks(check_name, ok) as (
  values
    ...
    (
      'actual_attendance has reconciliation columns',
      exists (
        select 1
        from information_schema.columns
        where table_schema = 'public'
          and table_name = 'actual_attendance'
          and column_name in ('default_attendance_id', 'student_id', 'actual_status', 'reconciliation_status')
        group by table_name
        having count(*) = 4
      )
    ),
