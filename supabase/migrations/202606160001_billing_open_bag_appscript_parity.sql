-- =============================================================================
-- Billing open-bag Apps Script parity
--
-- Adds normalized detail tables for the old InvoiceOpenBagSidebar behavior:
-- each student's bag line can preserve its own session slots and fee rows.
-- payment_bag_lines remains the yellow-sheet aggregate surface.
-- =============================================================================
...
create table if not exists public.payment_bag_line_sessions (
  ...
  foreign key (line_id, tenant_id) references public.payment_bag_lines(id, tenant_id) on delete cascade,
...
create table if not exists public.payment_bag_line_items (
  ...
  tenant_id uuid not null references public.tenants(id) on delete cascade,
  line_id uuid not null,
  item_type text not null check (item_type in ('tuition', 'book', 'misc', 'discount', 'carryover', 'adjustment')),
  label text,
...
begin
  foreach target_table in array detail_tables loop
